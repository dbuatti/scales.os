...
import { LogIn, Check, FileText } from 'lucide-react';
import { 
...
const DohnanyiPracticePanel: React.FC<DohnanyiPracticePanelProps> = ({ 
  ...
  const handleLogSnapshot = useCallback(() => {
    ...
    if (currentBPM > highestMasteredBPM) {
        updateExerciseMasteryBPM(currentExerciseBaseId, currentBPM);
  ...
  useEffect(() => {
      const newActivePracticeItem: ActivePracticeItem = {
          ...
          currentHighestBPM: highestMasteredBPM,
          isMastered: highestMasteredBPM >= DOHNANYI_BPM_TARGETS[DOHNANYI_BPM_TARGETS.length - 1],
      };
    ...
    <CardContent className="p-0 space-y-6">
        <div className="space-y-3 border p-4 rounded-lg border-primary/30 bg-secondary/50">
            ...
            <p className="text-xs text-muted-foreground italic mb-4 text-primary/70">
                Select the exercise you are currently practicing.
            </p>
            <ToggleGroup 
                type="single" 
            ...
            >
                {DOHNANYI_EXERCISES.map(exercise => (
                    <ToggleGroupItem 
                        key={exercise} 
                        value={exercise} 
                        className="data-[state=on]:bg-primary data-[state=on]:text-primary-foreground border border-border text-xs px-2 py-1 h-auto font-mono flex-1 min-w-[80px] focus-scale"
                    >
                        {exercise}
                    </ToggleGroupItem>
                ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4 p-4 rounded-lg border border-primary/30 bg-secondary/50 flex flex-col justify-between">
                <div className="space-y-1">
                    <Label className="text-lg font-semibold text-primary block font-mono text-glow">REFERENCE</Label>
                    <p className="text-[10px] text-muted-foreground uppercase tracking-widest">Sheet Music & Notes</p>
                </div>
                
                {selectedExercise === "Exercise I" ? (
                    <SheetMusicViewer 
                        title="Dohnányi Exercise I" 
                        imageUrl="https://rqesjpnhrjdjnrzdhzgw.supabase.co/storage/v1/object/public/assets/dohnanyi_ex1.png" 
                    />
                ) : (
                    <div className="flex items-center justify-center h-10 border border-dashed border-primary/20 rounded-md text-[10px] text-muted-foreground font-mono">
                        NO REFERENCE LOADED
                    </div>
                )}
            </div>
